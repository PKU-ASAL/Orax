...
// orax
Term Solver::mkTermBVCastToFloat(const Term& t){
  // FIXME: buggy
  assert(false);
  const unsigned int childNum = 1;
  cvc5::expr::NodeValue* nv = (expr::NodeValue*)std::malloc(sizeof(expr::NodeValue) + (sizeof(expr::NodeValue*) * childNum));
  if(nv == NULL) {
    throw std::bad_alloc();
  }
  auto d_nm = getNodeManager();
  nv->d_nchildren = childNum;
  nv->d_kind = kind::FLOATINGPOINT_TO_FP_IEEE_BITVECTOR;
  nv->d_id = d_nm->next_id++;// FIXME multithreading
  nv->d_rc = 0;
  d_nm->poolInsert(nv);
  Node n = Node(nv);
  Term res = Term(this, n);
  return res;
}
}  // namespace api
