...
// orax
bool fuzzSolveFirst = true;
#if CVC4_USE_SYMFPU
// FIXME: may cause many bugs
bool FPMergedInBV = false;
#else
bool FPMergedInBV = false;
#endif
extern theory::NodeSet oracleInfo;
namespace theory {
  extern bool fuzzingModeEnabled;
  extern bool fuzzSolveOnce;
  extern bool timeoutFlag;
  extern cvc5::Result fuzzResult;
  extern bool unsatFlag;
  extern bool reuseLastFuzz;
}
void initFuzzing(){
  cvc5::theory::fuzzingModeEnabled = true;
  cvc5::theory::fuzzSolveOnce = false;
  cvc5::theory::timeoutFlag = false;
  cvc5::theory::reuseLastFuzz = true;
  cvc5::theory::fuzzResult = cvc5::Result::UNSAT;
  cvc5::theory::unsatFlag = false;
}
void tempOpenFuzzing(){
  cvc5::theory::fuzzingModeEnabled = true;
  cvc5::theory::fuzzSolveOnce = false;
  cvc5::theory::timeoutFlag = false;
  cvc5::theory::reuseLastFuzz = true;
...
void tempCloseFuzzing(){
  cvc5::theory::fuzzingModeEnabled = false;
  cvc5::theory::fuzzSolveOnce = true;
  cvc5::theory::timeoutFlag = false;
  cvc5::theory::reuseLastFuzz = true;
  cvc5::theory::fuzzResult = cvc5::Result::UNSAT;
  cvc5::theory::unsatFlag = false;
...
void closeFuzzing(){
  ...
  cvc5::theory::fuzzSolveOnce = true;
  cvc5::theory::timeoutFlag = false;
  cvc5::theory::reuseLastFuzz = false;
  cvc5::theory::fuzzResult = cvc5::Result::UNSAT;
  cvc5::theory::unsatFlag = false;
}
static theory::NodeSet oracle_info_learned;
static std::map<std::string, std::pair<std::vector<unsigned int>, std::vector<bool>>> functionDeclarations; // function name, <function_declaration_sizes, function_declaration_is_float_vector> (e.g., (return value size, arg sizes))
static std::map<std::string, std::set<Node>> functionToNodes; // function name to node
static size_t apply_uf_cnt = 0;
static std::map<std::string, Node> var_names;
static theory::NodeSet findNodeInfo(const Node &nd) {
  ...
  for (; bi != be; ++bi) {
    ...
    if (node.getKind() == kind::VARIABLE) {
      if (var_names.find(node.toString()) == var_names.end())
        var_names[node.toString()] = node;
      else
        assert (var_names[node.toString()] == node);
    }
  }
  return ufs;
}
static void collectFunctionInfo(cvc5::smt::Assertions& as){
  static bool collected = false;
  if (collected)
    return;
  var_names.clear();
  // pass through assertions
  auto node_list = as.getAssertionList();
  std::set<std::string> function_declared;
  for (int i = 0; i < node_list->size(); ++i){
    Node tnode = (*node_list)[i];
    // find all APPLY_UF nodes
    theory::NodeSet ns = findNodeInfo(tnode);
    for (auto n: ns){
      apply_uf_cnt++;
      assert(n.getKind() == kind::APPLY_UF);
      std::string funcName = n.getOperator().toString();
      // store function node
      if (functionToNodes.find(funcName) == functionToNodes.end()){
        functionToNodes[funcName] = std::set<Node>();
      }
      functionToNodes[funcName].insert(n);
      ...
      assert(functionDeclarations.find(funcName) == functionDeclarations.end());
      ...
      functionDeclarations[funcName] = std::make_pair(function_declaration_sizes, function_declaration_is_float_vector);
  ...
  for (auto &info: functionDeclarations){
    llvm::errs() << "Function Name: " << info.first << "\n";
  }
  // print functionToNodes
  for (auto &info: functionToNodes){
    llvm::errs() << "Function Name: " << info.first << "\n";
    for (auto &node: info.second){
      llvm::errs() << "Node: " << node.toString() << "\n";
    }
  }
  #endif
}
static void learn_oracle_Info(cvc5::smt::Assertions& as, std::map<std::string, std::set<std::vector<uint64_t>>>& key_points, size_t free_uf_points, bool free_arg){
  oracle_info_learned.clear();
  size_t freed = 0;
  for (auto &info : key_points){
    ...
    assert(functionDeclarations.find(oracleName) != functionDeclarations.end());
    assert(functionToNodes.find(oracleName) != functionToNodes.end());
    assert(functionToNodes[oracleName].size() > 0);
    auto op = functionToNodes[oracleName].begin()->getOperator();
    auto function_declaration_sizes = functionDeclarations[oracleName].first;
    auto function_declaration_is_float_vector = functionDeclarations[oracleName].second;
    ...
    for (auto &keyPoint: keyPoints){
      ...
      if (!free_arg){
        if (freed < free_uf_points){
          freed++;
          Node freeVar = NodeManager::currentNM()->mkSkolem("free", ret.getType());
          Node eqNode = NodeManager::currentNM()->mkNode(kind::EQUAL, ufNode, freeVar);
          oracle_info_learned.insert(eqNode);
        }
        else {
    ...
    auto fnodes = functionToNodes[oracleName];
    for (auto & fnode: fnodes){
      ...
      // free points
      if (free_arg && freed < free_uf_points){
        freed++;
        std::vector<Node> pointConstraints;
        for (unsigned int i = 0; i < fnode.getNumChildren(); ++i){
          Node arg = fnode[i];
          Node freeVar = NodeManager::currentNM()->mkSkolem("free", arg.getType());
          Node eqNode = NodeManager::currentNM()->mkNode(kind::EQUAL, arg, freeVar);
          pointConstraints.push_back(eqNode);
        }
        Node pointConstraint = NodeManager::currentNM()->mkAnd(pointConstraints);
        constraints.push_back(pointConstraint);
      ...
      oracle_info_learned.insert(constraint);
    }
  }
  #ifdef ENABLE_CBF_DEBUG
  // print oracle_info_learned
  for (auto &info: oracle_info_learned){
    llvm::errs() << "Selected Oracle Info: " << info.toString() << "\n";
  }
  #endif
  return;
}
...
SmtEngine::SmtEngine(NodeManager* nm, Options* optr)
    : d_env(new Env(nm)),
      ...
      d_pp(nullptr),
      d_scope(nullptr),
      d_fuzzIterCount("driver::fuzzIters", 0)
{
  ...
  smtStatisticsRegistry()->registerStat(&d_fuzzIterCount);
}
...
{
  SmtScope smts(this);
  smtStatisticsRegistry()->unregisterStat(&d_fuzzIterCount);
  try
  {
    shutdown();
  ...
  {
    Warning5() << "CVC4 threw an exception during cleanup." << endl << e << endl;
  }
}
...
void SmtEngine::setInfo(const std::string& key, const std::string& value)
{
  ...
  else if (key == "smt-lib-version" && !options::inputLanguage.wasSetByUser())
  {
    ...
    {
      Warning5() << "SMT-LIB version " << value
...
cvc5::SExpr SmtEngine::getInfo(const std::string& key) const
{
  ...
  if (key == "all-statistics")
  {
    ...
    for (StatisticsRegistry::const_iterator i =
             d_env->getStatisticsRegistry()->begin();
                                   ...
                                   bool isEntailmentCheck)
{
  try
  {
    ...
    closeFuzzing();
    #ifdef ENABLE_CBF_DEBUG
    std::cout << "d_asserts: " << std::endl;
    auto al = d_asserts->getAssertionList();
    ...
    #endif
    collectFunctionInfo(*d_asserts.get());
    Result r = Result::SAT;
    // orax
    if (r.asSatisfiabilityResult().isSat() == Result::SAT && options::fuzzSolve())
    {
      // pushing smt_engine internal state
      TheoryEngine* te = d_smtSolver->getTheoryEngine();
      size_t fuzzTrails = 0;
      Result tempRes = r;
      size_t MAX_ITERATION = 50;
      size_t MAX_TIME_S = 600;
      // starttime
      auto start = std::chrono::high_resolution_clock::now();
      initFuzzing();
      while(true) {
        if (fuzzTrails >= MAX_ITERATION) {
          break;
        }
        auto current = std::chrono::high_resolution_clock::now();
        auto duration = std::chrono::duration_cast<std::chrono::seconds>(current - start).count();
        if (duration >= MAX_TIME_S) {
          break;
        }
        #ifdef ENABLE_CBF_DEBUG
        ...
        d_fuzzIterCount.set(fuzzTrails);
        #ifdef ENABLE_CBF_DEBUG
        std::cout << "SMT Engine internal push..." << std::endl;
        #endif
        d_state->userPush();

        tempOpenFuzzing();
        // call smt solver again with fuzz solver
        cvc5::cgen::AssertionListPtr = d_asserts.get();
	      tempRes = d_smtSolver->checkSatisfiability(*d_asserts.get(), assumptions, inUnsatCore, isEntailmentCheck);
        if (cvc5::theory::unsatFlag){
          tempRes = Result::UNSAT;
          break;
        }
        if (cvc5::theory::fuzzResult.asSatisfiabilityResult().isSat() != Result::SAT) {
          tempRes = Result(Result::SAT_UNKNOWN, Result::UNKNOWN_REASON);
          for (auto it = oracleInfo.begin(); it != oracleInfo.end(); ++it) {
            #ifdef ENABLE_CBF_DEBUG
            std::cout << *it << std::endl;
            #endif
            Node tn = Node(*it);
            d_asserts->assertFormula(tn, inUnsatCore);
          }
          // clear oracleInfo
          #ifdef ENABLE_CBF_DEBUG
          std::cout << "d_asserts new: " << std::endl;
          auto al = d_asserts->getAssertionList();
          for (auto it = al->begin(); it != al->end(); ++it) {
            std::cout << *it << std::endl;
          }
          #endif
          tempCloseFuzzing();
          // use SMT Solver to solve
          cvc5::cgen::AssertionListPtr = d_asserts.get();
          tempRes = d_smtSolver->checkSatisfiability(*d_asserts.get(), assumptions, inUnsatCore, isEntailmentCheck);
          if (tempRes.asSatisfiabilityResult().isSat() == Result::SAT){
            break;
          }
          Result tempRes2 = Result(Result::SAT_UNKNOWN, Result::UNKNOWN_REASON);
          auto key_points = oracleKeyPoints;
          std::vector<std::map<std::string, uint64_t>> var_values_collection;
          bool free_arg = true;
          bool free_arg_this_round = free_arg;
          for (int free_uf_points = 0; free_uf_points < 2 * apply_uf_cnt; free_uf_points++) {
            d_state->userPop();
            d_state->userPush();
            #ifdef ENABLE_CBF_DEBUG
            std::cout << "d_asserts in free section: " << std::endl;
            auto al = d_asserts->getAssertionList();
            for (auto it = al->begin(); it != al->end(); ++it) {
              std::cout << *it << std::endl;
            }
            std::cout << "key points in free section: " << std::endl;
            for (auto &kp : key_points) {
              std::cout << "Oracle Name: " << kp.first << std::endl;
              for (auto &kpv : kp.second) {
                std::cout << "Oracle Value: " << kpv.back() << std::endl;
                std::cout << "Oracle Args: ";
                for (int i = 0; i < kpv.size() - 1; i++) {
                  std::cout << kpv[i] << " ";
                }
                std::cout << std::endl;
              }
            ...
            #endif
            free_arg_this_round = free_arg;
            if (free_arg_this_round){
              // free args
              // 0 -> 1, 1 -> 1, 2 -> 2, 3 -> 2, 4 -> 3, 5 -> 3, ...
              size_t _cnt = ((size_t)free_uf_points >> 1) + 1;
              learn_oracle_Info(*d_asserts.get(), key_points, _cnt, free_arg);
              free_arg = false;
            }
            else {
              // free return value
            ...
            for (auto it = oracle_info_learned.begin(); it != oracle_info_learned.end(); ++it) {
              Node tn = Node(*it);
              d_asserts->assertFormula(tn, inUnsatCore);
            }
            #ifdef ENABLE_CBF_DEBUG
            std::cout << "d_asserts with key points in free section: " << std::endl;
            al = d_asserts->getAssertionList();
            for (auto it = al->begin(); it != al->end(); ++it) {
              std::cout << *it << std::endl;
            }
            #endif
            // check again
            cvc5::cgen::AssertionListPtr = d_asserts.get();
            tempRes2 = d_smtSolver->checkSatisfiability(*d_asserts.get(), assumptions, inUnsatCore, isEntailmentCheck);
            if (tempRes2.asSatisfiabilityResult().isSat() == Result::SAT) {
              std::map<std::string, std::vector<std::vector<uint64_t>>> oracle_models;
              for (auto &p : functionToNodes) {
                auto fname = p.first;
                auto fnodes = p.second;
                if (oracle_models.find(fname) == oracle_models.end()) {
                  oracle_models[fname] = std::vector<std::vector<uint64_t>>();
                }
                for (auto &fnode : fnodes) {
                  std::vector<uint64_t> tmodel;
                  // args
                  for (int i = 0; i < fnode.getNumChildren(); i++) {
                    auto argValue = getValue(fnode[i]);
                    if (argValue.getKind() == kind::CONST_BITVECTOR){
                      tmodel.push_back(argValue.getConst<BitVector>().getValue().getUnsignedLong());
                    }
                    else {
                      Assert (false);
                    }
                  ...
                  // result
                  auto fvalue = getValue(fnode);
                  if (fvalue.getKind() == kind::CONST_BITVECTOR){
                    tmodel.push_back(fvalue.getConst<BitVector>().getValue().getUnsignedLong());
                  }
                  else {
                    Assert (false);
                  }
                  #ifdef ENABLE_CBF_DEBUG
                  // print the uf model
                  std::cout << "uf model of " << fname << ": " << std::endl;
                  std::cout << "result: " << tmodel.back() << " args: ";
                  assert(tmodel.size() > 0);
                  for (int i = 0; i < tmodel.size() - 1; i++) {
                    std::cout << tmodel[i] << " ";
                  }
                  std::cout << std::endl;
                  #endif
                  bool isKeyPoint = false;
                  assert(key_points.find(fname) != key_points.end());
                  if (key_points[fname].find(tmodel) != key_points[fname].end()) {
                    isKeyPoint = true;
                  }
                  if (!isKeyPoint) {
                    oracle_models[fname].push_back(tmodel);
                  }
                }
              }
              for (auto &p : oracle_models) {
                #ifdef ENABLE_CBF_DEBUG
                std::cout << "oracle models of " << p.first << ": " << std::endl;
                for (auto &model : p.second) {
                  std::cout << "result: " << model.back() << " args: ";
                  assert(model.size() > 0);
                ...
                #endif
                auto fname = p.first;
                auto fmodels = p.second;
                for (auto &fmodel : fmodels) {
                  assert (fmodel.size() > 0);
                  assert (oracleDatabase.find(fname) != oracleDatabase.end());
                  auto ret_value = fmodel.back();
                  auto args = fmodel;
                  auto select_value = ret_value;
                  args.pop_back();
                  assert (cvc5::cgen::jitgen != nullptr);
                  uint64_t f_result;
                  auto success = cvc5::cgen::jitgen->evaluate_uf(fname, f_result, args, functionDeclarations[fname].first, functionDeclarations[fname].second);
                  if (success) {
                    if (oracleDatabase[fname].find(f_result) == oracleDatabase[fname].end()) {
                      oracleDatabase[fname][f_result] = std::set<std::vector<uint64_t>>();
                    }
                    if (oracleDatabase[fname][f_result].find(args) == oracleDatabase[fname][f_result].end()) {
                      oracleDatabase[fname][f_result].insert(args);
                    }
                    #ifdef ENABLE_CBF_DEBUG
                    std::cout << "newly found real model of " << p.first << ": " << std::endl;
                    std::cout << "result: " << f_result << " args: ";
                    for (auto &arg : args) {
                      std::cout << arg << " ";
                    }
                    std::cout << std::endl;
                  ...
                  if (oracleDatabase[fname].find(ret_value) != oracleDatabase[fname].end()) {
                    if (success && f_result == ret_value) {
                      key_points[fname].insert(fmodel);
                      continue;
                    }
                    else if (oracleDatabase[fname][ret_value].find(args) != oracleDatabase[fname][ret_value].end()) {
                      // FIXME: redundant?
                      key_points[fname].insert(fmodel);
                      continue;
                    }
                    ...
                    uint64_t closest_ret_value = INT64_MAX;
                    for (auto &p : oracleDatabase[fname]) {
                      if (abs((double)p.first - (double)ret_value) < abs((double)closest_ret_value - (double)ret_value)) {
                        closest_ret_value = p.first;
                      }
                    }
                    assert (closest_ret_value != INT64_MAX);
                    assert (oracleDatabase[fname].find(closest_ret_value) != oracleDatabase[fname].end());
                    select_value = closest_ret_value;
                  }
                  assert(oracleDatabase[fname][select_value].size() > 0);
                  std::vector<uint64_t> closest_model;
                  double min_distance = DBL_MAX;
                  for (auto &argsVector : oracleDatabase[fname][select_value]) {
                    assert (argsVector.size() == args.size());
                    // Euclidean distance
                    double distance = 0;
                    for (int i = 0; i < args.size(); i++) {
                      distance += pow(args[i] - argsVector[i], 2);
                    }
                    distance = sqrt(distance);
                    if (distance < min_distance) {
                      min_distance = distance;
                      closest_model = argsVector;
                    }
                  }
                  // add to key_points
                  closest_model.push_back(select_value);
                  key_points[fname].insert(closest_model);
                  #ifdef ENABLE_CBF_DEBUG
                  std::cout << "closest model of " << fname << ": " << std::endl;
                  std::cout << "result: " << select_value << " args: ";
                  for (auto &arg : closest_model) {
                    std::cout << arg << " ";
                  }
                  std::cout << std::endl;
                  #endif
              ...
              std::map<std::string, uint64_t> var_values;
              for (auto &p : var_names) {
                auto var_name = p.first;
                auto var_node = p.second;
                auto var_value = getValue(var_node);
                if (var_value.getKind() == kind::CONST_BITVECTOR){
                  var_values[var_name] = var_value.getConst<BitVector>().getValue().getUnsignedLong();
                }
                else {
                  Assert (false);
                }
              }
              var_values_collection.push_back(var_values);
            }
            // if unsat: keep adding new free args to UF
          }
          cvc5::cgen::ModelsFromSMTSolver = var_values_collection;
        }
        else {
          tempRes = cvc5::Result::SAT;
          #ifdef ENABLE_CBF_DEBUG
          std::cout << "Theory solvers : "
              << termcolor::red << tempRes.asSatisfiabilityResult() << termcolor::reset
              << std::endl;
          #endif
          break;
        }
        #ifdef ENABLE_CBF_DEBUG
        std::cout << "SMT Engine internal pop..." << std::endl;
        #endif
        d_state->userPop();
        closeFuzzing();
      }
      // without cleaning the oracleInfo, the final cleanup step will be problematic
      // cleanup
      oracleInfo.clear();
      functionDeclarations.clear();
      functionToNodes.clear();
      var_names.clear();
      oracle_info_learned.clear();
      cvc5::cgen::jitgen = nullptr;
      cvc5::cgen::AssertionListPtr = nullptr;
      r = tempRes;
      if (r != Result::SAT) {
        if (unsatFlag) {
          r = Result::UNSAT;
          ...
          throw ModalException("Failed to find a model");
        }
      }
    }

    setResultStatistic(r.toString());
...
bool SmtEngine::validateModel(std::map<Node, Node> partmodel) {
  // orax
  assert(false);
  TheoryEngine* te = d_smtSolver->getTheoryEngine();
  uf::TheoryUF* ufTheory = static_cast<uf::TheoryUF*>(te->theoryOf(THEORY_UF));
  (ufTheory->d_cg)->dumpPartialModel(partmodel);
  bool status = AUFBVWrafl::validatePartialModel("partmodel.bin");
  // we already have executable file in the current directory, just need to
  // generate the model for validation
  return status;
}
...
Node SmtEngine::getQuantifierElimination(Node q, bool doFull, bool strict)
{
  ...
  {
    Warning5() << "Unexpected logic for quantifier elimination " << logic
              << endl;
  }
