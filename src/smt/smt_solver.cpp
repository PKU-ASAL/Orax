                                      ...
                                      bool isEntailmentCheck)
{
  ...
  // if (options::fuzzSolve() && result.asSatisfiabilityResult() == Result::UNSAT)
  // {
  //   // check if the UF theory is in unsat and check for the reason

  //   TheoryEngine* te = getTheoryEngine();
  //   theory::Theory* ufTheory = te->theoryOf(theory::TheoryId::THEORY_UF);

  //   // TODO: add fuzz solver's check to LAST_EFFORT for to be consistent
  //   if (te->isInconflict() && ufTheory->getTheoryState()->isInConflict())
  //   {
  //     theory::InferenceId ufConflictReason =
  //         ufTheory->getTheoryState()->conflictReason();

  //     std::cout << termcolor::blue
  //               << "THEORY_UF conflict status : " << ufConflictReason
  //               << termcolor::reset << std::endl;

  //     if (ufConflictReason == theory::InferenceId::UF_FUZZ_SOLVER_UNSAT)
  //       result = Result(Result::UNSAT);
  //     else if (ufConflictReason == theory::InferenceId::UF_FUZZ_SOLVER_UNKNOWN)
  //       result = Result(Result::SAT_UNKNOWN, Result::UNKNOWN_REASON);
  //   }
  ...
  {
    if (options::fuzzSolve())
    {
      std::cerr << termcolor::red << termcolor::bold
                << "--global-negated definition is incomplete along with "
                   "--fuzz-solver"
                << termcolor::reset << std::endl;
