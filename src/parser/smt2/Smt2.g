...
@declarations {
  ...
  | /* cb-function declaration */
    DECLARE_CBFUN_TOK { PARSER_STATE->checkThatLogicIsSet(); }
    symbol[name,CHECK_NONE,SYM_VARIABLE]
    { PARSER_STATE->checkUserSymbol(name); }
    LPAREN_TOK sortList[sorts] RPAREN_TOK
    sortSymbol[t,CHECK_DECLARED]
    { Debug("parser") << "declare fun: '" << name << "'" << std::endl;
      ...
      if( PARSER_STATE->sygus() )
      {
        PARSER_STATE->parseErrorLogic("declare-cbf are not allowed in sygus "
                                      "version 2.0");
      }
      else
      {
  ...
  | /* oracle declaration */
    DECLARE_ORACLE_TOK { PARSER_STATE->checkThatLogicIsSet(); }
    symbol[name,CHECK_NONE,SYM_VARIABLE]
    { PARSER_STATE->checkUserSymbol(name); }
    LPAREN_TOK sortList[sorts] RPAREN_TOK
    sortSymbol[t,CHECK_DECLARED]
    { Debug("parser") << "declare fun: '" << name << "'" << std::endl;
      ...
      if( PARSER_STATE->sygus() )
      {
        PARSER_STATE->parseErrorLogic("declare-oracle are not allowed in sygus "
                                      "version 2.0");
      }
      else
      {
...
simpleSymbolicExprNoKeyword[std::string& s]
  ...
  | tok=(ASSERT_TOK | CHECK_SAT_TOK | CHECK_SAT_ASSUMING_TOK | DECLARE_FUN_TOK | DECLARE_CBFUN_TOK | DECLARE_ORACLE_TOK
        | DECLARE_SORT_TOK
...
DECLARE_CBFUN_TOK : 'declare-cbf';
DECLARE_ORACLE_TOK : 'declare-oracle';
