...
void TheoryEngine::check(Theory::Effort effort)
{
  // orax
  // DEBUG: print facts
  bool enable_cbf_debug = false;
  #ifdef ENABLE_CBF_DEBUG
  std::cerr <<
  "=============================================================\n" <<
  "TheoryEngine::check\n" <<
  "=============================================================\n";
  std::cerr << "\033[36m" << "* Facts:\n" << "\033[0m";
  enable_cbf_debug = true;
  #endif
  // spendResource();
...
#define CVC4_FOR_EACH_THEORY_STATEMENT(THEORY)                      \
  ...
  {                                                                 \
    if (enable_cbf_debug)                                           \
      theoryOf(THEORY)->debugPrintFacts();                           \
  ...
  try
  {
    ...
    if (Theory::fullEffort(effort) && !d_inConflict && !needCheck())
    {
      ...
      if (!d_inConflict)
      {
        // TODO : making an explicit last call check for uf_theory solver
        if (d_logicInfo.isQuantified())
        {
    ...
    std::vector<NodeTheoryPair>& explanationVector)
{
  ...
  uf::TheoryUF* ufTheory =
        static_cast<uf::TheoryUF*>(theoryOf(THEORY_UF));
  ...
  // store some conflicts from THEORY_UF itself, decide whether to add them
  theory::NodeSet secondaryConflicts;
  bool hasPrimaryConflicts = false;
  ...
  // selecting some more conflicts from THEORY_UF
  // if (!hasPrimaryConflicts) {
  //   for (auto nfi = secondaryConflicts.begin(); nfi != secondaryConflicts.end(); ++nfi) {
  //     Node t = (*nfi).notNode();
  //     ufTheory->storeConflictClause(t);
  //     std::cout << termcolor::magenta << "saving: " << t << termcolor::reset << std::endl;
  //   }
  // }
...
bool TheoryEngine::storeForFuzzer(Node nd) {
  if (nd.getKind() == kind::EQUAL) {
    if (nd[0].getKind() != kind::CONST_RATIONAL && nd[1].getKind() != kind::CONST_RATIONAL)
      if (nd[0].getKind() != kind::CONST_BITVECTOR && nd[1].getKind() != kind::CONST_BITVECTOR)
      return true;
  }

  if (nd.getKind() == kind::NOT) {
    nd = nd[0];
    return storeForFuzzer(nd);
  }
