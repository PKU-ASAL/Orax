    ...
    EqProof* eqp) const
{
  ...
  if (!canExplain) {
    Warning5() << "Can't explain equality:" << std::endl;
    Warning5() << d_nodes[t1Id] << " with find " << d_nodes[getEqualityNode(t1Id).getFind()] << std::endl;
    Warning5() << d_nodes[t2Id] << " with find " << d_nodes[getEqualityNode(t2Id).getFind()] << std::endl;
  }
  Assert(canExplain);
#endif
