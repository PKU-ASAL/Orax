...
TheoryUF::TheoryUF(context::Context* c,
    ...
    : Theory(THEORY_UF, c, u, out, valuation, logicInfo, pnm, instanceName),
      ...
      d_fuzzTime("driver::totalFuzzTime"),
      ...
      d_notify(d_im, *this)
{
  ...
  d_inferManager = &d_im;
  fuzzingMode = false;
  fuzzerUsedOnce = false;
  fuzzConflict = false;
  fuzzStatus = FuzzStat();
  fuzzSolveStatus = AUFBVFuzzStat();
  smtStatisticsRegistry()->registerStat(&d_fuzzTime);
}

TheoryUF::~TheoryUF() {
  smtStatisticsRegistry()->unregisterStat(&d_fuzzTime);
}
...
void TheoryUF::postCheck(Effort level)
{
  ...
  // if (options::fuzzSolve() && level == EFFORT_FULL && numAssertions())
  // {
  //   FuzzStat fuzzStat = fuzzCheck();
  //   if (!fuzzStat.crashed)
  //   {
  //     InferenceId inferId = (fuzzStat.hasInputVars)
  //                               ? InferenceId::UF_FUZZ_SOLVER_UNKNOWN
  //                               : InferenceId::UF_FUZZ_SOLVER_UNSAT;

  //     d_inferManager->conflict(*(facts_begin()), inferId);
  //   }
  // }
}

void TheoryUF::resetFuzzer()
{
  fuzzerUsedOnce = false;
  fuzzConflict = false;
  fuzzAsserts.clear();
  // clean up the model learned
}

bool TheoryUF::isVarAssignment(const Node& nd) {
  if (nd.getKind() != kind::EQUAL) return false;
  else if (nd[0].getKind() == kind::VARIABLE && nd[1].getKind() == kind::CONST_BITVECTOR) return true;
  else if (nd[1].getKind() == kind::VARIABLE && nd[0].getKind() == kind::CONST_BITVECTOR) return true;
  else if (nd[0].getKind() == kind::VARIABLE && nd[1].getKind() == kind::CONST_RATIONAL) return true;
  else if (nd[1].getKind() == kind::VARIABLE && nd[0].getKind() == kind::CONST_RATIONAL) return true;
  else return false;
}

void TheoryUF::unifyMultipleAssignments() {

  std::cout << "**UNIFIY MULTIPLE ASSIGNMENT" << std::endl;
  std::map<Node, Node> unimap;
  NodeSet newConflictSet;

  for (auto it = conflictsLearnt.begin(); it != conflictsLearnt.end(); ++it) {
    if (!isVarAssignment(*it)) {
      newConflictSet.insert(*it);
      continue;
    }

    std::cout << "ASSIGNMENT NODE:" << *it << std::endl;
    Node vnd = Node(*it);
    if (vnd[0].getKind() == kind::VARIABLE) { vnd = vnd[0]; }
    else { vnd = vnd[1]; }

    auto vid = unimap.find(vnd);
    if (vid == unimap.end()) {
      unimap[vnd] = (*it);
    }
    else {
      Node tnd = unimap[vnd].orNode((*it));
      unimap[vnd] = tnd;
    }

    std::cout << "ERASING: " << vnd << std::endl;
    conflictsLearnt.erase(vnd);
  }

  for (auto mit : unimap) {
    std::cout << "inserting: " << mit.second << std::endl;
    newConflictSet.insert(mit.second);
  }
  conflictsLearnt = newConflictSet;
}



FuzzStat TheoryUF::fuzzCheck()
{
  assert(false);
  return FuzzStat();
  // logging d_facts contents into a text file
  // std::ofstream ofd("facts.txt", std::ofstream::out);
  // this->printFacts(ofd);
  // ofd << std::endl;
  // ofd.close();

  // std::ofstream ofs("target.c", std::ofstream::out);
  // d_cg->dump(ofs);
  // ofs.close();

  // // TODO: figure out a way to get the library paths
  // // FIXME : Throw error if target lib is not present in the path.
  // // Implement this inside wrafl submodule.

  // std::cout << "target lib : " << options::targetLib() << std::endl;
  // Wrafl afl("./target.c", options::targetLib(), "./indir", "./outdir", 600);

  // std::cout << "Running fuzzcheck()..." << std::endl;
  // TimerStat::CodeTimer checkTimer(d_fuzzTime);

  // // if target code has input variable then no fuzzing
  // // execute once and observe crash
  // bool simplecheck = d_cg->hasInputVars();
  // FuzzStat fuzzstatus = afl.fuzz(simplecheck);
  // fuzzstatus.hasInputVars = d_cg->hasInputVars();

  // if (fuzzstatus.crashed)
  // {
  //   // bool validation = afl.validateCrash(fuzzstatus);
  //   // if (validation == false)
  //   // {
  //   //   std::cerr << "Crash validation failed. Exiting..." << std::endl;
  //   //   exit(1);
  //   // }
...
TrustNode TheoryUF::ppRewrite(TNode node, std::vector<SkolemLemma>& lems)
{
  ...
  if (k == kind::HO_APPLY)
  {
    ...
    {
      std::stringstream ss;
      ss << "Partial function applications are not supported in default mode, "
            "try --uf-ho.";
...
void TheoryUF::check(Effort level)
{
  ...
  while (!done() && !d_theoryState->isInConflict())
  {
    ...
    // extract variable terms from fact and store it in varset
    // extractVars(fact);
...
void TheoryUF::storeConflictClause(Node const& nd) {
  assert(false);
  conflictsLearnt.insert(nd);
}
