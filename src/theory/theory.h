...
class Theory
{
 ...
 protected:
  ...
  const LogicInfo& getLogicInfo() const { return d_logicInfo; }
 ...
 public:
   ...
   */
  context::Context* getSatContext() const { return d_satContext; }
   ...
   */
  virtual void setUserAttribute(const std::string& attr,
                                Node n,
  ...
  /**
   * This is a utility function for constructing a copy of the currently shared
   * terms in a queriable form.  As this is
   */
  ...
  // orax
public:
  std::unique_ptr<cvc5::cgen::AUFBVJitGenerator> d_cg;
  AUFBVFuzzStat fuzzSolve();
  void clearFacts() {
    d_facts.clear();
  }
}; /* class Theory */
