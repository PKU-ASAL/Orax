                                         ...
                                         std::shared_ptr<ProofNode> proof)
{
  ...
  if (isProofEnabled())
  {
    if (proof == nullptr)
    {
      Warning5() << "CircuitPropagator: Proof is missing for " << n << std::endl;
      Assert(false);
    }
    else
    {
      ...
      {
        Warning5() << "CircuitPropagator: Incorrect proof: " << expected
       ...
       ++i)
  {
    ...
    if (atom)
    {
      ...
      if (isProofEnabled())
      {
        ...
        {
          Warning5() << "CircuitPropagator: Proof is missing for " << lit
                    << std::endl;
