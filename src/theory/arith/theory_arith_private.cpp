...
bool TheoryArithPrivate::entireStateIsConsistent(const string& s){
  ...
  for(var_iterator vi = var_begin(), vend = var_end(); vi != vend; ++vi){
    ...
    if(!d_partialModel.assignmentIsConsistent(var)){
      ...
      Warning5() << s << ":" << "Assignment is not consistent for " << var << d_partialModel.asNode(var);
      ...
      d_partialModel.printModel(var);
      Warning5() << s << ":" << "Assignment is not integer for integer variable " << var << d_partialModel.asNode(var);
      if(d_tableau.isBasic(var)){
        Warning5() << " (basic)";
      }
      Warning5() << endl;
  ...
  for(var_iterator vi = var_begin(), vend = var_end(); vi != vend; ++vi){
    ...
    if(!d_partialModel.assignmentIsConsistent(var)){
      if(!d_errorSet.inError(var)){
        ...
        d_partialModel.printModel(var);
        Warning5() << "Unenqueued var is not consistent for " << var <<  d_partialModel.asNode(var);
        if(d_tableau.isBasic(var)){
          Warning5() << " (basic)";
        }
        Warning5() << endl;
        ...
        d_partialModel.printModel(var);
        Warning5() << "Initial var is not consistent for " << var <<  d_partialModel.asNode(var);
        if(d_tableau.isBasic(var)){
          Warning5() << " (basic)";
        }
        Warning5() << endl;
  ...
  if((upperBound && d_partialModel.strictlyLessThanUpperBound(basic, bound)) ||
    ...
    if(bestImplied != NullConstraint){
      ...
      if(bestImplied->negationHasProof()){
        Warning5() << "the negation of " <<  bestImplied << " : " << endl
