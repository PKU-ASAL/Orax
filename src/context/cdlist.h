...
protected:
   ...
   */
  T* d_list;
  std::vector<T*> d_list_history_segments;
  std::vector<size_t> d_list_history_segments_sizes;
...
protected:
  ...
  void restore_list() {
    ...
    Assert(d_list_history_segments.size() == d_list_history_segments_sizes.size());
    size_t sizeAlloc_needed = 0;
    for (size_t i = 0; i < d_list_history_segments.size(); i++) {
      sizeAlloc_needed += d_list_history_segments_sizes[i];
    }
    if (sizeAlloc_needed > d_sizeAlloc) {
      // Allocate a new array with double the size
      size_t newSize = sizeAlloc_needed;
      if(newSize > d_allocator.max_size()) {
        newSize = d_allocator.max_size();
        Assert(newSize > d_sizeAlloc)
            << "cannot request larger list due to allocator limits";
      ...
      size_t offset = 0;
      for (size_t i = 0; i < d_list_history_segments.size(); i++) {
        std::memcpy(newList + offset, d_list_history_segments[i], sizeof(T) * d_list_history_segments_sizes[i]);
        offset += d_list_history_segments_sizes[i];
      }
      d_allocator.deallocate(d_list, d_sizeAlloc);
      d_list = newList;
      d_sizeAlloc = newSize;
      Assert(offset == sizeAlloc_needed);
      d_size = sizeAlloc_needed;
    }
    else {
      size_t offset = 0;
      for (size_t i = 0; i < d_list_history_segments.size(); i++) {
        std::memcpy(d_list + offset, d_list_history_segments[i], sizeof(T) * d_list_history_segments_sizes[i]);
        offset += d_list_history_segments_sizes[i];
      }
      d_size = sizeAlloc_needed;
    }
    d_list_history_segments.clear();
    d_list_history_segments_sizes.clear();
...
public:
  ...
  void clear() {
    ...
    d_list_history_segments.push_back(d_list);
    d_list_history_segments_sizes.push_back(d_size);
    history_cached = true;
    // create a new list
    d_list = NULL;
    d_size = 0;
